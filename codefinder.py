...
class CodeFinder(object):
    ...
    def get_code_files(self):
        ...
        for extension in self.file_extensions:
          files += self.__get_files_with_extension_in_dir(self.path, extension)

        git_ignore_file_array = self.__get_files_with_extension_in_dir(self.path, ".gitignore")
        ...
        for file in files:
            if self._is_git_ignored(file, git_ignore_file):
                result.remove(file)
